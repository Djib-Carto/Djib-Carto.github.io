var map, featureList, boroughSearch = [], poteauSearch = [], compteurSearch = [] , zoneSearch = [], zone4Search = [], subwayLinesSearch = [];
...
/* Boucle sur subwayLines et ajoute uniquement les lignes visibles dans la carte */
subwayLines.eachLayer(function (layer) {
  if (map.hasLayer(subwayLines)) {
    if (layer.getBounds && map.getBounds().intersects(layer.getBounds())) {
      var center = layer.getBounds().getCenter();
      var nomLigne = layer.feature.properties.name || "Sans nom";
      var couleur = layer.feature.properties.color || "#000";

      var rowId = "row-" + L.stamp(layer); // identifiant unique

      // Ajouter à la liste latérale
      $("#feature-list tbody").append(
        '<tr class="feature-row" id="' + rowId + '" data-id="' + L.stamp(layer) + '">' +
          '<td style="vertical-align: middle;">' +
            '<span style="display:inline-block;width:16px;height:16px;background:' + couleur + ';border-radius:4px;"></span>' +
          '</td>' +
          '<td class="feature-name">' + nomLigne + '</td>' +
          '<td style="vertical-align: middle;"><i class="fa fa-chevron-right pull-right"></i></td>' +
        '</tr>'
      );

      // Gérer le clic (événement délégué plus performant si nécessaire)
      $("#" + rowId).on("click", function () {
        // Zoom sur la ligne
        map.fitBounds(layer.getBounds(), { padding: [20, 20] });

        // Affiche les infos dans un modal
        var content = "<table class='table table-striped table-bordered table-condensed'>" +
                      "<tr><th>Nom</th><td>" + nomLigne + "</td></tr></table>";
        $("#feature-title").html(nomLigne);
        ...
        // Rétablir le style après un court délai
        setTimeout(() => {
          subwayLines.resetStyle(layer);
        }, 1500);
      });
    }
  }
...
var subwayLines = L.geoJson(null, {
  ...
  onEachFeature: function (feature, layer) {
    if (feature.properties) {
      var content = "<table class='table table-striped table-bordered table-condensed'>" + "<tr><th>Nom</th><td>" + feature.properties.name   + "<table>";
      layer.on({
        click: function (e) {
...
subwayLines.eachLayer(function (layer) {
  if (layer.getBounds && layer.feature && layer.feature.properties) {
    var center = layer.getBounds().getCenter();

    subwayLinesSearch.push({
      name: layer.feature.properties.name || "Sans nom",
      address: layer.feature.properties.name || "Sans nom",
      source: "subwayLines",
      id: L.stamp(layer),
      lat: center.lat,
      lng: center.lng
...
$.getJSON("data/cable.geojson", function (data) {
...
attributionControl.onAdd = function (map) {
  ...
  div.innerHTML = "<span class='hidden-xs'>Tous droits réservés à Djib Carto| </span>";
  return div;
};
...
var subwayLinesBH = new Bloodhound({
    name: "subwayLines",
    datumTokenizer: function (d) {
      return Bloodhound.tokenizers.whitespace(d.name);
    },
    queryTokenizer: Bloodhound.tokenizers.whitespace,
    local: subwayLinesSearch,
    limit: 10
  });
  ...
  $("#searchbox").typeahead({
    ...
    name: "subwayLines",
    displayKey: "name",
    source: subwayLinesBH.ttAdapter(),
    templates: {
      header: "<h4 class='typeahead-header'><img src='fibre.png' width='35' height='30'>&nbsp; Fibre Optique</h4>",
      suggestion: Handlebars.compile(["{{name}}<br>&nbsp;<small>{{address}}</small>"].join(""))
    },
  },{
    name: "GeoNames",
...
if (datum.source === "subwayLines") {
  ...
  // 1) S’assurer que la couche est bien affichée
  if (!map.hasLayer(subwayLines)) {           // change ici si ta variable s’appelle autrement
    map.addLayer(subwayLines);
  }

  // 2) Retrouver le layer correspondant à l'id
  let targetLayer = null;
  subwayLines.eachLayer(function (layer) {
    if (L.stamp(layer) === datum.id) {
      targetLayer = layer;
    }
  });

  // 3) Si on l’a trouvé, zoom + popup
  if (targetLayer) {
    map.fitBounds(targetLayer.getBounds(), { padding: [20, 20] }); // zoom sur toute la ligne
    targetLayer.fire("click");                                      // ouvre la modale/popup
  }
}
